...
enum switch_mask {SW_GEN_TC = PIN0_bm, SW_COM = PIN1_bm, SW_ANLG = PIN2_bm, SW_ON = PIN3_bm};
...
// == Button 0 pressed
		if (((SWITCHPORTL.IN & SW_GEN_TC) | pressed) == 0x00) {		
...
// == Button 1 pressed
		} else if (((SWITCHPORTL.IN & SW_COM) | pressed) == 0x00) {	
			_delay_ms(5);
			pressed = 1;
...
// == Button 2 pressed
		} else if (((SWITCHPORTL.IN & SW_ANLG) | pressed) == 0x00) {
			_delay_ms(5);
			pressed = 1;
...
// == Button 3 pressed
		} else if (((SWITCHPORTL.IN & SW_ON) | pressed) == 0x00) { 
			_delay_ms(5); // Debounce switch
			pressed = 1; // Used for toggling.
		...
		} else if ( (SWITCHPORTL.IN & (SW_GEN_TC | SW_COM | SW_ANLG | SW_ON)) == 0x0F) {
			pressed = 0;
			sleep();
