...
int main(void)
{
	...
	SWITCHPORTL.INT0MASK = SWITCHMASK_ACTIVE;
	SWITCHPORTL.PIN0CTRL = (SWITCHPORTL.PIN0CTRL & ~PORT_ISC_gm) | PORT_ISC_FALLING_gc;
	...
	while (1) {
		...
		if ((SWITCHPORTL.IN & SWITCHMASK_ACTIVE) == 0x00)
		{
		...
		} else if ((SWITCHPORTL.IN & SWITCHMASK_IDLE) == 0x00)	
	    {	
