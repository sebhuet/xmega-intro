...
int main( void )
{
  ...
  /* Just as for the timer in PWM mode, the output channels the AWEX will use must be enabled
  ...
  TCE0_init();        //initialize the timer
  ...
  while(1)
  {
    ...
    {     
       ...
       * pattern for the output. This ensure that for BLDC motor control for example, the commutation sequence 
       * can be synchronized with the UPDATE condition for the time */
