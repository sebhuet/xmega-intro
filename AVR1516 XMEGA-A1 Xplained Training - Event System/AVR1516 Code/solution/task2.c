...
int main( void ){
   ...
   * for each event channel. This is the same as in task1. */
   ...
   /* Add code that select SWITCHPORT0 as event channel 0 multiplexer input.
    * This is done by setting the coresponding  PORT_PIN0 group mask for the CH0MUX register */

  //Add code here.
  #if defined( __XMEGA_A1_XPLAINED__ )
	EVSYS.CH0MUX = EVSYS_CHMUX_PORTD_PIN0_gc;
  #elif defined( __XPLAIN__ )
	EVSYS.CH0MUX = EVSYS_CHMUX_PORTF_PIN0_gc;
  #elif
	#error: please select a board definition in board.h
  #endif
  /* The port pin used for input capture must be configred with the
  ...
  */
  ...
  SWITCHPORTL.PIN0CTRL |= PORT_ISC_BOTHEDGES_gc;   //Configure pin to sense both edges.
  SWITCHPORTL.DIRCLR = PIN0_bm;                      //Clear direction register to ensure that pin 0 is input
  ...
  while (1) {
    ...
    LEDPORT.OUT = ~capture_value;
     ...
     /* Each pin low level generate an event and input capture. This will lead to buffer overflow if the CCA register is
     * not read in time.
